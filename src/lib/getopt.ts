...
export interface HelpTextGeneratorOptions {
    linePrefix: string
    lineSeparator: string
}


export const generateHelpText = (options: OptionSet, generatorOptions?: HelpTextGeneratorOptions) => options.flatMap((option) => {
...
}).join(generatorOptions?.lineSeparator ?? "\n")

export function withOptions(options: OptionSet) {
    return {
        generateShort: () => generateShort(options),
        generateLong: () => generateLong(options),
        generateHelpText: (generatorOptions?: HelpTextGeneratorOptions) => generateHelpText(options, generatorOptions),
